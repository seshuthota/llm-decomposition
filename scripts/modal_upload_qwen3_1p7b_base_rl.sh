...
REMOTE_SUBPATH="${2:-/qwen3-1.7b-base}"
...
root = Path.home() / ".cache/huggingface/hub/models--Qwen--Qwen3-1.7B-Base"
...
if [[ ! -d "${LOCAL_MODEL_DIR}" ]]; then
  ...
  echo "Run ./scripts/cache_qwen3_1p7b_base_rl.sh first, or pass the snapshot path explicitly."
  exit 1
fi
