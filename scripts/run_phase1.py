#!/usr/bin/env python3
"""Prepare and optionally dry-run the first experiment cycle."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare phase-1 baseline runs.")
    parser.add_argument(
        "--manifest",
        default="configs/phase1/phase1_manifest.json",
        help="Path to the phase-1 manifest JSON file.",
    )
    parser.add_argument(
        "--run-id",
        action="append",
    ...
    parser.add_argument(
        "--prepare-only",
        action="store_true",
        help="Prepare run directories and metadata without extra console detail.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    if not args.prepare_only:
        print(f"Prepared {len(prepared_runs)} phase-1 runs.")
